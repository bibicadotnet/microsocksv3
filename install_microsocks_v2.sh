...
while true; do
    ...
    echo -e "${YELLOW}• Would you like to enable Cloudflare WARP? (y/n) [default: n]${NC}"
    echo -e "${WHITE}"
    read -p "Enable WARP?: " ENABLE_WARP
    echo -e "${NC}"
    ENABLE_WARP=$(echo "$ENABLE_WARP" | tr '[:upper:]' '[:lower:]')
    if [[ -z "$ENABLE_WARP" || "$ENABLE_WARP" == "n" || "$ENABLE_WARP" == "no" ]]; then
        TUNNEL_PROTOCOL="none"
        break
    elif [[ "$ENABLE_WARP" == "y" || "$ENABLE_WARP" == "yes" ]]; then
        while true; do
            echo -e "\n${YELLOW}Select WARP Tunnel Protocol:${NC}"
            echo -e "  1) WireGuard (Kernel-level, recommended, high performance)"
            echo -e "  2) MASQUE (Userspace HTTP/3 QUIC, bypasses UDP blocks)"
            echo -e "${WHITE}"
            read -p "Enter choice (1 or 2) [default: 1]: " WARP_CHOICE
            echo -e "${NC}"
            if [[ -z "$WARP_CHOICE" || "$WARP_CHOICE" == "1" ]]; then
                TUNNEL_PROTOCOL="wireguard"
                break
            elif [[ "$WARP_CHOICE" == "2" ]]; then
...
CHECK_IP=$(curl -4 -s --connect-timeout 10 --max-time 15 --proxy $PROXY_URL https://ifconfig.me 2>/dev/null || echo "")
